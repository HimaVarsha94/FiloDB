...
class ParserSpec extends FunSpec with Matchers {
  ...
  it("Should be able to make logical plans for Series Expressions") {
    val queryToLpString = Map(
      ...
      "predict_linear(http_requests_total{job=\"api-server\"}[5m], 50)" ->
        "PeriodicSeriesWithWindowing(RawSeries(IntervalSelector(1524855688000,1524855988000),List(ColumnFilter(job,Equals(api-server)), ColumnFilter(__name__,Equals(http_requests_total))),List()),1524855988000,1000000,1524855988000,300000,PredictLinear,List(50.0))",
